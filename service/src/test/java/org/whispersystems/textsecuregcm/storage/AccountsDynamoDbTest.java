...
class AccountsDynamoDbTest {
  ...
  void testRetrieve() {
    ...
    Optional<Account> retrievedFirst = accountsDynamoDb.get(uuidFirst);
    Optional<Account> retrievedSecond = accountsDynamoDb.get(uuidSecond);
  ...
  @Disabled("In Diskuv you cannot overwrite an account since the UUID is stable")
  @Test
  void testOverwrite() {
    Device  device  = generateDevice (1                                            );
  ...
  private Account generateAccount(String number, UUID uuid) {
    Device device = generateDevice(1);
    return generateAccount(uuid.toString(), uuid, Collections.singleton(device));
  }
  ...
  private Account generateAccount(String number, UUID uuid, Set<Device> devices) {
    ...
    return new Account(uuid.toString(), uuid, devices, unidentifiedAccessKey);
  }
