...
public class HmacDrbgTest {
  private byte[] hexToBytes(String s) {
    try {
      return Hex.decodeHex(s.toUpperCase());
  ...
  public void testHmacDrbgNistCase0() {
    ...
    HmacDrbg drbg = new HmacDrbg(ByteUtil.combine(entropy, nonce), null);
  ...
  public void testHmacDrbgNistWithPersonalizationCase0() {
    ...
    HmacDrbg drbg = new HmacDrbg(ByteUtil.combine(entropy, nonce), personalizationString);
    byte[] out1 = new byte[1024 / 8];
    drbg.nextBytes(out1);
    byte[] out2 = new byte[1024 / 8];
    drbg.nextBytes(out2);
